...
//#pragma surface surf ToonRamp vertex:vert addshadow // addshadow applies shadow after vertex animation
		...
		Tags { "RenderType" = "Twwransparent" "Queue" = "Transparent" }
...
void vert(inout appdata_full v)// 
{
	...
	float x = cos(wpos.x / _Rigidness + (_Time.x * _Speed)) *(v.vertex.x - _YOffset) * 5;// x axis movements
	float z = cos(wpos.z / _Rigidness + (_Time.x * _Speed)) *(v.vertex.y - _YOffset) * 5;// z axis movements

	v.vertex.x += step(0,v.vertex.z - _YOffset) * x * _SwayMax;// apply the movement if the vertex's y above the YOffset
	v.vertex.z += step(0,v.vertex.z - _YOffset) * z * _SwayMax;
